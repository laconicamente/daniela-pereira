...
$(".lx-contact form input[type='button']").on("click",function(){
	...
	var fullname = $(".lx-contact form input[name='nome']");
	if(fullname.val() == ""){
		fullname.after("<span>Este campo precisa ser preenchido.</span>").css("border-right","3px solid #a94442");
	}
	
	var email = $(".lx-contact form input[name='email']");
	var patt = /^[a-z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,4}$/;
	if(!patt.test(email.val())){
		email.after("<span>Email ínvalido.</span>").css("border-right","3px solid #a94442");
	}
	
	var txtarea = $(".lx-contact form textarea");
	if(txtarea.val() == ""){
		txtarea.after("<span>Este campo precisa ser preenchido.</span>").css("border-right","3px solid #a94442");
	}

	if($(".lx-contact form span").length == 0){
		var url = "contato.php?fullname="+fullname.val()+"&email="+email.val()+"&message="+txtarea.val();
		var posting = $.post( url );
		posting.done(function( data ) {
			$(".lx-contact-saved").html(data);	
			$(".lx-contact form input[name='nome']").val("");
